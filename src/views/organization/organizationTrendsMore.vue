			...
			<div id="mescroll" class="mescroll">
				<div>
					<van-list v-model="loading2" :finished="finished2">
						<van-card :thumb="item.url" v-for="item in publishedList" :key="item.id" @click="gotoArticle(item.id)">
							<template #title>
  ...
  methods: {
    ...
    getData() {
      axios({
        method: "get",
        url:
          "/organization/organizationDynamics/syncList?modularType=1&pageSize=" +
          //   (Math.pow(2, 24) - 1),
